...
uint16_t Modbus_CRC16(uint8_t *buf, uint8_t len) {		// function to calculate the crc based on algorithm
    uint16_t crc = 0xFFFF;
...
//	holding_registers[2] = 0x1212;
	HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_0);  //yellow led
	...
	if (function_code == 0x03){
		...
		//uint8_t reply[5 + 2 * register_count];
	    reply[0] = slave_address; // slave address
	    reply[1] = function_code; // function code
		...
		uint16_t start_address = (modbus_frame[2] << 8) | modbus_frame[3];
		values[0] = (modbus_frame[4] << 8) | modbus_frame[5];
		holding_registers[start_address] = values[0];
		HAL_UART_Transmit_IT(&huart2, modbus_frame, 8);
	}
		...
		for (int i=0; i<register_count; i++){
			values[i] = (modbus_frame[7+2*i] << 8) | modbus_frame[8+2*i];
